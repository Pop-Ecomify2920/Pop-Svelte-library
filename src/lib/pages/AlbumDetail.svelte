...
<RequireAuth>
  <div class="h-full bg-immich-bg dark:bg-immich-dark-bg">
    ...
    <div class="flex items-start justify-between gap-4">
      ...
      <button type="button" class="px-4 py-2 rounded-lg bg-immich-primary text-primary-foreground" on:click={openAdd}>
        Manage photos
      </button>
    </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-1">
        {#each albumPhotos as photo (photo.id)}
          <div class="relative group overflow-hidden rounded-lg">
            ...
            <button
              type="button"
              class="absolute top-2 left-2 px-2 py-1 rounded-lg bg-black/50 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity"
              on:click={() => {
                if (album) {
                  updateAlbum(albumId, {
            ...
            <button
              type="button"
              class="absolute top-2 right-2 px-2 py-1 rounded-lg bg-black/50 text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity"
              on:click={() => {
      ...
      <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="auth-card max-w-3xl w-full">
          <div class="flex items-center justify-between">
            ...
            <button type="button" class="px-3 py-1.5 rounded-lg bg-secondary" on:click={() => (addOpen = false)}>Close</button>
          </div>
          ...
          <div class="mt-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-1 max-h-[55vh] overflow-auto">
            {#each $uploadedPhotos as p (p.id)}
              <button
                type="button"
                class={`relative overflow-hidden rounded-lg ${selectedToAdd.includes(p.id) ? 'ring-2 ring-immich-primary' : 'ring-0'}`}
          ...
          <div class="mt-6 flex justify-end gap-2">
            <button type="button" class="px-4 py-2 rounded-lg bg-secondary" on:click={() => (addOpen = false)}>Cancel</button>
            <button type="button" class="px-4 py-2 rounded-lg bg-immich-primary text-primary-foreground" on:click={saveAdd}>
              Save
            </button>
          </div>
        </div>
